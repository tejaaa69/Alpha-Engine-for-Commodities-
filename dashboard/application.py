...
Unified Alchemist Dashboard.
Directly interfaces with the MLflow Model Registry and LangGraph Agent.
...
@st.cache_resource
def get_tracker(_cfg):
    return AlchemistTracker(_cfg)

@st.cache_resource
def load_prod_model(_tracker):
    """Loads the model dynamically from the MLflow Registry, not local files."""
    return _tracker.load_production_model()

@st.cache_data(ttl=1800)
def load_data(_cfg, symbol):
    """Loads feature store and raw prices."""
    store = FeatureStore(_cfg)
    features_df = store.build(symbol, force_rebuild=False)
    price_df = MarketIngestion(_cfg).load(symbol)
...
def render_sidebar(cfg):
    ...
    symbol = st.sidebar.selectbox("Asset", [cfg["assets"]["gold"], cfg["assets"]["silver"]], index=0)
    threshold = st.sidebar.slider("Execution Threshold", 0.50, 0.80, 0.55, 0.01)
...
def tab_live_signal(cfg, symbol, threshold, model, features_df, price_df):
    if model is None:
        st.error(f"No Production model found for {symbol} in MLflow. Run `main.py` first.")
        return

    feature_cols = [c for c in model.feature_cols if c in features_df.columns]
    ...
    if "regime_code" in features_df.columns:
        # Map back from our elite Volatility regimes
        code = features_df["regime_code"].iloc[-1]
        regime_map = {0: "LOW_VOL (Bull)", 1: "MID_VOL (Transition)", 2: "HIGH_VOL (Crisis/Bear)"}
    ...
    c1.metric("Date", str(latest_date.date()))
    ...
    with st.spinner("Calculating live SHAP Log-Odds..."):
        # Dynamically generate SHAP (Replaces the broken local CSV load)
        background = features_df[feature_cols].tail(100)
        ...
        fig = go.Figure(go.Bar(
            x=local_df["shap_value"], y=local_df["feature"],
            orientation="h", marker_color=colors,
            text=[f"{v:.2f}" for v in local_df["feature_value"]], textposition='auto'
        ))
        fig.update_layout(
        ...
        st.caption("Green = Pushing probability UP. Red = Pushing probability DOWN. Text inside bars = Actual Feature Value.")


def tab_backtest(cfg, symbol, model, features_df, price_df):
    if model is None:
        st.info("No model found. Run training pipeline."); return

    # Pull true OOS metrics directly from MLflow
    client = mlflow.MlflowClient()
    versions = client.get_latest_versions(cfg["mlflow"]["registered_model_name"], stages=["Production", "Staging"])
    
    if not versions:
        st.warning("No metrics found in Registry."); return
    ...
    st.divider()
    st.info("To view the full trade log and equity curve, check your MLflow Dashboard UI. The metrics above represent the strict, Purged Walk-Forward Out-Of-Sample performance.")


def tab_regime(features_df, price_df):
    if "regime_code" not in features_df.columns:
        st.info("No regime data available."); return

    regime_series = features_df["regime_code"].tail(1000)
    price_series  = price_df["close"].reindex(regime_series.index).ffill()

    # Define strict color mappings for our Volatility clusters
    cmap = {
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=price_series.index, y=price_series.values, name="Price", line=dict(color="white",width=1.5)))
    
    prev, start_b = None, None
    for date, reg in regime_series.items():
        if reg != prev:
            if prev is not None:
                fig.add_vrect(x0=start_b, x1=date, fillcolor=cmap.get(prev, "gray"), opacity=1.0, layer="below", line_width=0)
            start_b, prev = date, reg
            
    fig.update_layout(title="Asset Price + HMM Volatility Regimes", template="plotly_dark", height=450)
    st.plotly_chart(fig, use_container_width=True)


def tab_agent(symbol):
    st.subheader(f"💬 Ask Alchemist ({symbol})")
    st.caption("Agent uses memory. It remembers previous questions in this session.")
    ...
    # Chat Input
    if prompt := st.chat_input("Ask about the macro outlook, SHAP drivers, or WGC documents..."):
...
def main():
    ...
    features_df, price_df = load_data(cfg, symbol)

    t1, t2, t3, t4 = st.tabs([
        "📡 Live Signal", "📈 Backtest OOS", "🌊 Regime Monitor", "🤖 Research Agent"
    ])
    
    with t1: tab_live_signal(cfg, symbol, threshold, model, features_df, price_df)
    with t2: tab_backtest(cfg, symbol, model, features_df, price_df)
    with t3: tab_regime(features_df, price_df)
    with t4: tab_agent(symbol)
